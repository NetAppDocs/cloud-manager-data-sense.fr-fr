...
Voir <<Préparez le système hôte Linux,Préparation du système hôte Linux>> et <<Vérifier les prérequis BlueXP et Data Sense,Vérification des prérequis>> Avant de déployer Cloud Data Sense, vous devez consulter la liste complète des exigences.
...
* Vérifiez que votre environnement répond aux exigences requises <<Vérifier les prérequis BlueXP et Data Sense,autorisations et connectivité>>.
